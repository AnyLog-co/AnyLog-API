...
def __lower_case(configs:dict):
    ...
    updated_configs = {}
    for key in configs:
        updated_key = key.lower().strip()
        try:
            updated_configs[updated_key] = int(configs[key].strip())
        except:
            updated_configs[updated_key] = configs[key].strip()
            if configs[key].capitalize() == "True":
                updated_configs[updated_key] = True
