...
def deploy_node(anylog_conn:anylog_connector.AnyLogConnector, configuration:dict, exception:bool=False):
    """
    Deploy an AnyLog query node
    :process:
        1. connect to system_query (used for querying data) if configured
    ...
    if len(blockchain.get_policy(anylog_conn=anylog_conn, policy_type="query",
        ...
        new_policy = blockchain.create_node_policy(anylog_conn=anylog_conn, policy_type="query", configuration=configuration,
                                                   cluster_id=None, exception=exception)
        try:
            json_policy = json.dumps(new_policy)
        except Exception as error:
            print(f"Failed to convert query node policy to JSON format (Error: {error})")
        else:
