...
def build_increments_query(table_name:str, time_interval:str, units:int, time_column:str, value_columns,
                           ...
                           where_condition:str="", exception:bool=False)->str:
    """
    ...
    sql_cmd = f"SELECT increments({time_interval}, {units}, {time_column}), MIN({time_column}) AS min_ts, MAX({time_interval}) AS max_ts,"

    if all(False is x for x in [calc_min, calc_avg, calc_max, row_count]):
        if exception is True:
            print(f"Unable to generate query. Missing aggeregate function(s)")
        return None
...
def query_data(conn:anylog_connector.AnyLogConnector, db_name:str, output_format:str='json', stat:bool=True,
    ...
    :args:
        ...
        output_format:str - result output formmat
            -> json
            -> table
            -> json:list
