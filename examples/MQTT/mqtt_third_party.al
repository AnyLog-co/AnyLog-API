...
run mqtt client where broker=driver.cloudmqtt.com and user=mqwdtklv and password=uRimssLO4dIo and port=18975 and log=false and topic=(name=third-party-broker and dbms="bring [dbms]" and table="bring [table]" and column.timestamp.timestamp="bring [ts]" and column.value.float="bring [values]" and column.value_id.str="bring [other]")
